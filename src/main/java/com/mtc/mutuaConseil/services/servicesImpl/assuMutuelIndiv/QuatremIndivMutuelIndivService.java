...
@Service
public class QuatremIndivMutuelIndivService extends PageElementInteraction implements LaunchedService {

    private final Logger log = LoggerFactory.getLogger(QuatremIndivMutuelIndivService.class);
    public static WebDriver driver;
    private final String source = "QuatremIndivMutuelIndivService";
    private static Actions actions;
    private final TypeAssuranceService typeAssuranceService;

    public QuatremIndivMutuelIndivService(TypeAssuranceService typeAssuranceService){
        this.typeAssuranceService = typeAssuranceService;
    }

    @Override
    public Tarif getResultFrom(Compte c, FluxData flux) {
        Tarif tarifQuatremIndivMutuelIndiv = TarifUtils.createDefaultTarif(c, typeAssuranceService, 2L);;
        tarifQuatremIndivMutuelIndiv.setNom(c.getNomFournisseur());
        ...
        try {
            ...
            log.info("cout {}", cout);
            tarifQuatremIndivMutuelIndiv.setMontant(cout);
            String screenshotBytes = captureScreenshot(driver, tarifQuatremIndivMutuelIndiv.getNom(), false, tarifQuatremIndivMutuelIndiv);
            if (screenshotBytes != null) {
                tarifQuatremIndivMutuelIndiv.setCaptureImg(screenshotBytes);
            }
            tarifQuatremIndivMutuelIndiv.setExecution(true);
        } catch (Exception e) {
            log.error("An error occurred", e);
            tarifQuatremIndivMutuelIndiv.setErreur(e.getMessage());
            String screenshotBytesErreur = captureScreenshot(driver, tarifQuatremIndivMutuelIndiv.getNom(), true, tarifQuatremIndivMutuelIndiv);
            tarifQuatremIndivMutuelIndiv.setCaptureImgErreur(screenshotBytesErreur);
            tarifQuatremIndivMutuelIndiv.setEtape("");
        } finally {
            driver.quit();
        }
        return tarifQuatremIndivMutuelIndiv;
    }
    ...
    public void connexion(Compte c) {
        ...
        buttonCookies.click();
        WebElement inputLogin = waitForElement(driver, By.id("login"), 10, 1);
        inputLogin.sendKeys(c.getUsername());
        WebElement inputPassword = waitForElement(driver, By.id("pwd"), 10, 1);
