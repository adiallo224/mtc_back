...
@Service
public class ECAMutuelProService extends PageElementInteraction implements LaunchedService {

    private final Logger log = LoggerFactory.getLogger(ECAMutuelProService.class);
    public static WebDriver driver;
    private final String source = "ECAMutuelProService";
...
//                 scrollDown(driver, 0, 500);
    ...
    public void connexion(Compte c) {
        ...
        buttonCookies.click();
        InformationsUser.infoLogin(driver, By.id("login-name"), 10, 2, c.getUsername());
        InformationsUser.infoPassword(driver, By.id("login-password"), 10, 2, c.getPassword());
        waitThread(2);
        WebElement buttonLogin = waitForElement(driver, By.xpath("//button[normalize-space()='- SE CONNECTER -']"), 20, 2);
        buttonLogin.click();
    }
